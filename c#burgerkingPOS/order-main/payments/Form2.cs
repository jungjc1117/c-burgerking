    ...
    public partial class Form2 : Form
    {
        ...
        public void InventoryHistoryChange()
        {
            try
            {
                if (db.OpenConnection())
                {
                    string query = $"INSERT INTO inventory_history (menu_id, menu_name, sold_quantity, time) " +
                                    $"SELECT om.menu_id, m.menu_name, om.quantity, p.payment_time " +
                                    $"FROM `order` o " +
                                    $"JOIN order_menu om ON o.order_id = om.order_id " +
                                    $"JOIN menu m ON om.menu_id = m.menu_id " +
                                    $"JOIN payment p ON o.order_id = p.order_id " +
                                    $"WHERE NOT EXISTS " +
                                    $"(SELECT 1 FROM inventory_history ih WHERE ih.menu_id = om.menu_id AND ih.time = p.payment_time);";
